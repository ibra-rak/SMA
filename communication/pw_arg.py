...
class ArgumentModel(Model):
    def __init__(self):
        ...
        agent_1.generate_preferences(list_items1)
        agent_2.generate_preferences(list_items2,[CriterionName.PRODUCTION_COST, CriterionName.DURABILITY,CriterionName.NOISE, CriterionName.ENVIRONMENT_IMPACT, CriterionName.CONSUMPTION])

        self.schedule.add(agent_1)
        self.schedule.add(agent_2)

        self.dispute=True
        first_message=Message(agent_1.get_name(), agent_2.get_name(), MessagePerformative.PROPOSE, agent_1.get_preference().most_preferred(self.list_item) )
        agent_1.send_message(first_message)
        self.turn=0
        self.mode=True #on aboutit forcément à un accord ? True si les agents font des concessions (ne considèrent plus dans leur calcul de top choix les items deja eliminés)

        self.items_not_concerned=self.list_item.copy()
    ...
    def step(self): 
        ...
        agent0 = self.schedule.agents[0]
        agent1 = self.schedule.agents[1]
        ...
        if type(mess.get_content())==type(Argument(True,"")):
            item,decision,_=mess.get_content().argument_parsing()
        ...
        if self.mode:
                    comparative=self.items_not_concerned.copy()
        else:
                    comparative=self.list_item
        ...
        if item in self.items_not_concerned:
            self.items_not_concerned.remove(item)
