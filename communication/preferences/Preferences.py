...
class Preferences:
    ...
    def most_preferred(self, item_list):
        ...
        for x in item_list:
            if self.is_preferred_item(x,best_item) :
                best_item=x
        return best_item
    ...
    def is_item_among_top_10_percent(self, item, item_list):
        ...
        scores={}
        for x in item_list:
            scores[x]=x.get_score(self)

        scores=dict(sorted(scores.items(), key=lambda p: p[1])) #tri du dict par le score
        cles=list(scores.keys()) #clés triées
        cles.reverse()

        is_top_item=False
        top_index=len(cles)//10
        if top_index==0:
            return cles[0] == item
        else :
            for k in range(top_index-1):
                if cles[k]==item :
                    is_top_item = True
        return is_top_item
